...
def save_one_buffer(args, save_buffer, env_name, from_start=False):
    ...
    path_name = '../../buffer/' + x_env_name + \
        '/buffer_' + str(args.save_buffer_id) + '/'
    if os.path.exists(path_name):
        random_name = '../../buffer/' + x_env_name + \
            '/buffer_' + str(random.randint(10, 1000)) + '/'
...
def run_sequential(args, logger):
    ...
    if args.is_save_buffer:
        raise('all save_buffer functions have error')
    ...
    if args.is_batch_rl:
        ...
        if args.is_from_start:
            x_env_name += '_from_start/'
        path_name = '../../buffer/' + x_env_name + \
            '/buffer_' + str(args.load_buffer_id) + '/'
    ...
    while runner.t_env <= args.t_max:
        ...
        if not args.is_batch_rl:
            ...
            if args.is_save_buffer:
                ...
                if save_buffer.is_from_start and save_buffer.episodes_in_buffer == save_buffer.buffer_size:
                    ...
                    save_one_buffer(args, save_buffer,
                                    env_name, from_start=True)
        ...
        for _ in range(args.num_circle):
            if buffer.can_sample(args.batch_size):
                ...
                if args.is_batch_rl:
                    runner.t_env += int(th.sum(episode_sample['filled']).cpu(
                    ).clone().detach().numpy()) // args.batch_size
        ...
        if args.env == 'mmdp_game_1' and \
            ...
            for i in range(args.n_actions):
                for j in range(args.n_actions):
                    new_actions = th.Tensor([i, j]).unsqueeze(
                        0).repeat(args.episode_limit + 1, 1)
                    if i == 0 and j == 0:
                        rew = th.Tensor([1, ])
                    else:
                        rew = th.Tensor([0, ])
                    if i == 1 and j == 1:
                        new_obs = th.Tensor([1, 0]).unsqueeze(0).unsqueeze(
                            0).repeat(args.episode_limit, args.n_agents, 1)
                    else:
                        new_obs = th.Tensor([0, 1]).unsqueeze(0).unsqueeze(
                            0).repeat(args.episode_limit, args.n_agents, 1)
                    # Truncate batch to only filled timesteps
                    max_ep_t = episode_sample.max_t_filled()
                    episode_sample = episode_sample[:, :max_ep_t]
                    ...
                    learner.train(episode_sample, runner.t_env,
                                  episode, show_demo=True, save_data=(i, j))
        ...
        if (args.env == 'matrix_game_1' or args.env == 'matrix_game_2' or args.env == 'matrix_game_3') and \
            ...
            for i in range(args.n_actions):
                for j in range(args.n_actions):
                    ...
                    episode_sample['actions'][0, :, :, 0] = new_actions
                    new_actions_onehot = th.zeros(
                        episode_sample['actions'].squeeze(3).shape + (args.n_actions,)).cuda()
                    new_actions_onehot = new_actions_onehot.scatter_(
